...
class TelemetryMetricResponse(BaseModel):
    """Serialization representation of a time-series telemetry metric record."""
    model_config = ConfigDict(from_attributes=True)

    metric_id: UUID = Field(..., description="Universally unique metric record identifier.")
    asset_id: UUID = Field(..., description="Foreign key reference to the origin compute asset.")
    event_time: Optional[datetime] = Field(None, description="Timestamp of physical metric sampling occurrence (UTC).")
    cpu_utilization: float = Field(..., description="Recorded CPU utilization percentage (0.0 to 100.0).")
    diagnostic_payload: Dict[str, Any] = Field(..., description="Extensible JSON attribute dictionary.")


class TelemetryMetricsListResponse(BaseModel):
    """Canonical collection wrapper for Telemetry Metrics query responses."""
    status: str = "success"
    data: List[TelemetryMetricResponse]
    pagination: PaginationMetadata


class WinlogResponse(BaseModel):
    """Serialization representation of a Windows Event Log record."""
    model_config = ConfigDict(from_attributes=True)

    log_id: UUID = Field(..., description="Unique log event processing record ID.")
    asset_id: UUID = Field(..., description="Host asset that generated the native .evtx stream event.")
    occurrence_time: Optional[datetime] = Field(None, description="Precise timestamp generated by origin OS eventing.")
    event_id: int = Field(..., description="Canonical Windows Event Log numeric identifier.")
    severity_level: str = Field(..., description="Categorized runtime criticality.")
    evtx_metadata: Dict[str, Any] = Field(..., description="Deserialized event properties and forensic extensions.")


class WinlogListResponse(BaseModel):
    """Canonical collection wrapper for Windows Event Log query responses."""
    status: str = "success"
    data: List[WinlogResponse]
    pagination: PaginationMetadata
